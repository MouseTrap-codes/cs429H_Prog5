...
/******************************************************************************
 * Macros used in Pass 1 to detect expansions.
 ******************************************************************************/
...
/* --- Validation routines for instructions (brr, mov, etc.) --- */
/* For brevity, these are implemented as in previous versions.
   (Assume validate_brr, validate_mov, validate_instruction_immediate,
   and is_valid_instruction_pass1 are implemented here exactly as before.)
   For our purposes, the key is that Pass 1 validates each non-comment, non-directive line.
*/

// [Assume validate_brr, validate_mov, validate_instruction_immediate,
//  is_valid_instruction_pass1 are defined exactly as in our previous versions.]

/* For brevity in this full source code, I include them verbatim below: */
...
/******************************************************************************
 * parseMacro: Expand macros in Pass 2.
 ******************************************************************************/
static void parseMacro(const char *line, FILE *fout) {
    ...
    // Halt now expands to "priv r0, r0, r0, 0"
...
/******************************************************************************
 * PASS 2: Generate the .tk file.
 * This version merges consecutive code segments.
 ******************************************************************************/
static void pass2(const char *infile, const char *outfile) {
    FILE *fin = fopen(infile, "r");
    if (!fin) { perror("pass2: fopen input"); exit(1); }
    FILE *fout = fopen(outfile, "w");
    if (!fout) { perror("pass2: fopen output"); fclose(fin); exit(1); }
    
    int last_directive = -1; // -1 = none, 0 = code, 1 = data.
    char line[1024];
    while (fgets(line, sizeof(line), fin)) {
        line[strcspn(line, "\n")] = '\0';
        trim(line);
        ...
        if (is_macro_line(line))
            ...
            fprintf(fout, "\t%s\n", line);
        last_directive = -1;  // Reset so new directives will be output.
    }
    fclose(fin);
    fclose(fout);
}
...
void populateTinkerInstruction() {
    ...
    // Privileged: halt expansion uses "priv r0, r0, r0, 0"
...
/******************************************************************************
 * parse_and_assemble_file:
 * Reads the .tk file and, depending on the current section (.code vs .data),
 * converts lines into 32-bit binary instructions or 64-bit binary data items.
 ******************************************************************************/
void parse_and_assemble_file(const char *input_filename, const char *output_filename) {
    FILE *fp = fopen(input_filename, "r");
    if (!fp) { perror("Error opening input file"); return; }
    FILE *out = fopen(output_filename, "w");
    if (!out) { perror("Error opening output file"); fclose(fp); return; }
    
    int current_section = 0; // 0 = code, 1 = data (default code)
    char line[1024];
...
/******************************************************************************
 * main
 * This version accepts two arguments: an input assembly file and a final output file.
 * A temporary .tk file is created for Pass 2.
 ******************************************************************************/
int main(int argc, char *argv[]) {
    ...
    // Pass 1: Validate and build label map.
    pass1(argv[1]);
    
    // Create a temporary file for the intermediate .tk output.
    ...
    // Pass 2: Expand macros and substitute labels into the .tk file.
    ...
    // Final assembly: convert the .tk file into binary code.
